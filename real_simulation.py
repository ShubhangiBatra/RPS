...
class RealSimulation():
        ...
        while (vid in traci.vehicle.getIDList()) or (self._step==0):
            ...
            try: 
                ...
                if prev_csid != curr_csid and curr_csid != "NULL":  
                    ...
                    current_state = self._get_state(vid, curr_csid)
                    action = self._choose_action(current_state)
                    
                    if self.getActualBatteryCapacity(vid) <= 150.0:
                        print("Overriding action given by model")
                        action = 1
    ...
    def _get_state(self, vid, curr_csid):
        ...
        state.extend([power1, cost1, cd1, power2, cost2, cd2, self.getActualBatteryCapacity(vid)])
